...
public static void main(String args[])
{
	...
	try{
	    ...
	    FileWriter logFW =  new FileWriter("Strength.log"); // runtime logs 

	    FileWriter logQueue =  new FileWriter("queue.log"); // runtime logs 

	    FileWriter logConst =  new FileWriter("constant.log"); // runtime logs 

	    FileWriter logNotConst =  new FileWriter("notConstant.log"); // runtime logs 

	    FileWriter logV =  new FileWriter("ArraysValues.log"); // runtime logs 

		//YOU NEED TO TYPE BELOW THE PATH TO valuesSlicing.log AND UNCOMMENT THE BELOW LINE.
		...
		while (fids.hasMoreElements())
		{
			...
			if (null != hashFlows.get(fid)){
				...
				double [] w = new double[valuesList.size()];
				double [] x = new double[valuesList.size()-1];
				double [] y = new double[valuesList.size()-1];

				for (int i = 0; i < valuesList.size(); i++)
				{
					LSValuePair item = (LSValuePair)valuesList.get(i);
					Number value1 = item.getValue1();
					w[i] = value1.doubleValue();
				}	
				...
				double [] t = new double[valueQueue.size()];
				double [] u = new double[valueQueue.size()-1];
				double [] v = new double[valueQueue.size()-1];
				...
				for (int i = 0; i <tmp_queue_size ; i++)
				{
					...
					Number value1 = item.getValue1();
					t[i] = value1.doubleValue();
				}	
				
				for (int i = 0; i < (tmp_queue_size-1); i++){
				    u[i]=t[i];
				}
				...
				if ((r != ERROR) && (eta != ERROR) && (entropy != ERROR) ){
			
				    histoEntropy[(int)(entropy/INC)]++;						
				    if (entropy > maxEntropy) maxEntropy = entropy;
				    if (Math.abs(entropy) < 0.000001) numZeroEntropy++;
				    sumEntropy += entropy;
			
				    histoETA[(int)(eta/INC)]++;						
				    if (eta > maxETA) maxETA = eta;
				    if (Math.abs(eta) < 0.000001) numZeroETA++;
				    sumETA += eta;
	
				    histoR[(int)(r/INC)]++;						
				    if (r > maxR) maxR = r;
				    if (Math.abs(r) < 0.000001) numZeroR++;
				    sumR += r;
	
				    if ( (Math.abs(entropy) < 0.000001) && (Math.abs(eta) < 0.000001) && (Math.abs(r) < 0.000001) ) numZeroAll++;
